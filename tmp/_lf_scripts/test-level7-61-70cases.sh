...
if [[ -z "$ADMIN_TOKEN" ]]; then
  ...
  print_case "Case 69 - Load test gi??? cao ??i???m" "$C69_INPUT" "ramp-up load keeps service stable and users can still book" "SKIP" "$C69_BODY"
  ...
  print_case "Case 69 - Load test gi??? cao ??i???m" "$C69_INPUT" "ramp-up remains stable: success each stage>=${CASE69_PEAK_MIN_SUCCESS_RATE}, stage3 p95<=${CASE69_PEAK_P95_LIMIT_MS}ms, stage3 rps near target, no sudden severe degradation" "200" "$C69_BODY"
  C69_OK=0
...
print_case "Case 70 - Auto scaling ho???t ?????ng" "$C70_INPUT" "status=200 AND scaled=true AND load success_rate>=${CASE70_MIN_SUCCESS_RATE} AND p95<=${CASE70_MAX_P95_MS}ms AND 5xx_rate<=${CASE70_MAX_5XX_RATE}" "$C70_STATUS" "$C70_BODY"
