...
print_case "Case 1 - register user" "201 + user_id exists + no validation error" "$C1_STATUS" "$C1_BODY"
if [[ "$C1_STATUS" == "201" ]] && [[ -n "$C1_USER_ID" ]] && ! echo "$C1_BODY" | contains_text 'VALIDATION_ERROR'; then
  mark_result 1 "1"
else
  mark_result 0 "1"
fi
...
print_case "Case 2 - login + JWT payload" "200 + access_token + payload has sub, exp" "$C2_STATUS" "$C2_BODY"
...
C3_INTEGRATION_OK=0
if [[ -n "$C3_ETA" ]] && [[ -n "$C3_FARE" ]]; then
  C3_INTEGRATION_OK=1
fi
print_case "Case 3 - create booking valid" "200/201 + REQUESTED/CONFIRMED + booking_id + ETA/Pricing" "$C3_STATUS" "$C3_BODY"
...
print_case "Case 4 - list bookings by user_id" "200 + list contains booking_id, status" "$C4_STATUS" "$C4_BODY"
...
print_case "Case 5 - driver online" "200 + status ONLINE" "$C5_STATUS" "$C5_BODY"
if [[ "$C5_STATUS" == "200" ]] && [[ "$C5_DRIVER_STATUS" == "ONLINE" ]]; then
  mark_result 1 "5"
else
  mark_result 0 "5"
fi
...
# Reuse Case 3 booking to avoid extra token/register flakiness.
echo "-- Running Case 6"
C6_STATUS="$C3_STATUS"
C6_BODY="$C3_BODY"
...
print_case "Case 7 - ETA estimate" "200 + eta > 0 and < 60" "$C7_STATUS" "$C7_BODY"
...
print_case "Case 8 - pricing estimate" "200 + price > base fare + surge >= 1" "$C8_STATUS" "$C8_BODY"
...
C10_VERIFY=$(call_json GET "/v1/auth/verify" "$C2_TOKEN")
C10_VERIFY_STATUS=$(echo "$C10_VERIFY" | sed -n '1p')
C10_VERIFY_BODY=$(echo "$C10_VERIFY" | sed '1d')
print_case "Case 10 - logout invalid token" "logout 200 + verify old token => 401" "$C10_LOGOUT_STATUS/$C10_VERIFY_STATUS" "$C10_LOGOUT_BODY"$'\n'"$C10_VERIFY_BODY"
if [[ "$C10_LOGOUT_STATUS" == "200" ]] && [[ "$C10_VERIFY_STATUS" == "401" ]]; then
  mark_result 1 "10"
else
  mark_result 0 "10"
fi
