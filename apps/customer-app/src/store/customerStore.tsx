...
export const CustomerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const assignDriverToActiveRide = useCallback((driverId: string) => {
    if (!driverId) return
    setActiveRide((prev) => {
      if (!prev) return prev
      return {
  ...
  const refreshActiveRide = useCallback(async () => {
    ...
    try {
      ...
      const nextPickup =
        typeof serverRide.pickupLabel === 'string' && serverRide.pickupLabel.trim()
          ? serverRide.pickupLabel
          : activeRide.pickup
      const nextDestination =
        typeof serverRide.dropoffLabel === 'string' && serverRide.dropoffLabel.trim()
          ? serverRide.dropoffLabel
