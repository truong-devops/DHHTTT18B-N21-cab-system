...
function RideTable({ rides = [], onSelect }) {
  const columns = [
    ...
    },
    {
      key: 'fare',
      header: 'Cước phí',
      render: (row) => (row.fare !== null && row.fare !== undefined ? row.fare : 'Không có')
    },
    {
      key: 'status',
      header: 'Trạng thái',
      render: (row) => <Badge variant={row.status === 'completed' ? 'success' : 'warning'}>{labelFrom(rideStatusLabels, row.status)}</Badge>
    },
    {
      key: 'action',
      header: 'Thao tác',
