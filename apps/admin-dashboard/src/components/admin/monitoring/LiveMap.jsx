  ...
  useEffect(() => {
    ...
    normalizedMarkers.forEach((marker) => {
      ...
      window.L.marker([marker.lat, marker.lng])
        .bindPopup(`${label}: ${marker.id}`)
        .addTo(layerRef.current)
    })

    if (
      normalizedMarkers.length > 0 &&
      (!fitRef.current.hasFit || fitRef.current.count !== normalizedMarkers.length)
    ) {
      const bounds = window.L.latLngBounds(
        normalizedMarkers.map((marker) => [marker.lat, marker.lng])
      )
    ...
    <div className="card">
      ...
      {!window.L && (
        <div className="text-muted">
          Không tải được thư viện bản đồ. Vui lòng kiểm tra kết nối mạng.
        </div>
      )}
