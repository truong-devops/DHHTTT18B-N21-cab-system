...
function Users() {
  ...
  return (
    <div>
      <PageHeader title="Người dùng" subtitle="Tìm kiếm, xem và quản lý tài khoản khách hàng và tài xế." />
      <div className="card">
        <div className="grid grid-3">
          ...
          <Select label="Vai trò" value={filters.role} onChange={(event) => setFilters((prev) => ({ ...prev, role: event.target.value }))}>
            <option value="">Tất cả</option>
      ...
      <UserDetailDrawer user={selected} onClose={() => setSelected(null)} onToggleStatus={handleToggleStatus} />
    </div>
  );
}
