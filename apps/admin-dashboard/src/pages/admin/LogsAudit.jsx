...
function LogsAudit() {
  ...
  useEffect(() => {
    async function load() {
      const [logResult, auditResult] = await Promise.all([auditService.listLogs(), auditService.listAudits()]);
  ...
  return (
    <div>
      <PageHeader title="Nhật ký & Kiểm toán" subtitle="Nhật ký hoạt động và sự kiện bảo mật của hệ thống." />
      <div className="tabs">
      ...
      <div className="card">{tab === 'logs' ? <LogTable logs={logs} onSelect={setSelectedLog} /> : <AuditTable audits={audits} />}</div>
