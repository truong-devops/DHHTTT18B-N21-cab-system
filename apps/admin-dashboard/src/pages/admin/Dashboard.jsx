...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const nextSnapshot = {
          rides: ridesResult.status === 'fulfilled' ? ridesResult.value.items || [] : [],
          ...
          payments: paymentsResult.status === 'fulfilled' ? paymentsResult.value.items || [] : []
        };
  ...
  const metrics = useMemo(() => {
    ...
    const completed = ridesWindow.filter((ride) => ride.status === 'completed').length;

    const cancelled = ridesWindow.filter((ride) => ride.status === 'cancelled').length;

    const activeDrivers = drivers.filter((driver) => driver.onlineStatus === 'ONLINE').length;

    const completedRides = ridesWindow.filter((ride) => ride.status === 'completed' && ride.createdAt && ride.statusUpdatedAt);
    ...
    const avgFare = paidPayments.length === 0 ? 0 : paidPayments.reduce((sum, payment) => sum + Number(payment.amount || 0), 0) / paidPayments.length;

    const paymentSuccess = payments.length === 0 ? 0 : (paidPayments.length / payments.length) * 100;
  ...
  return (
    <div>
      <PageHeader title="Tổng quan" subtitle="Tổng quan thời gian thực về chuyến đi, nhu cầu và tình trạng tài xế." />
