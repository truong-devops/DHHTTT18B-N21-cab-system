...
function Payments() {
  ...
  useEffect(() => {
    ...
    autoConfirmTimerRef.current = setTimeout(async () => {
      try {
        const updated = await paymentService.confirmDev(targetId, authToken.trim());
        if (updated) {
          setPayment(updated);
        }
  ...
  const nativeQrImage = useMemo(() => {
    ...
    if (payment?.payos?.qrCode && String(payment.payos.qrCode).startsWith('data:image')) {
  ...
  const handleCreate = async (event) => {
    ...
    try {
      const result = await paymentService.create(payload, idem, authToken.trim());
  ...
  const handleFetchQr = async () => {
    ...
    setError('');
    try {
      const result = await paymentService.getVietQr(payment.id, authToken.trim());
  ...
  const handleConfirmDev = async () => {
    ...
    setError('');
    try {
      const updated = await paymentService.confirmDev(payment.id, authToken.trim());
      if (updated) {
        setPayment(updated);
      }
      loadPayments();
  ...
  const statusOptions = useMemo(() => ['INITIATED', 'PROCESSING', 'PAID', 'FAILED', 'REFUNDED'], []);
  const isVietQrPayment = payment?.method === 'VIETQR';
  const isPayosPayment = payment?.method === 'PAYOS';
  const paymentAmountLabel = useMemo(() => formatMoney(payment?.amount, payment?.currency || 'VND'), [payment]);
  const statusText = payment?.status || 'INITIATED';
  const sheetHint = isPayosPayment ? 'Dung app ngan hang ho tro VietQR hoac app PayOS de quet.' : 'Quet ma de thanh toan nhanh.';
  ...
  const handleUpdateStatus = async (row) => {
    ...
    try {
      const updated = await paymentService.updateStatus(row.id, nextStatus, failureReason || null, authToken.trim());
      if (updated) {
        setPayments((prev) => prev.map((item) => (item.id === row.id ? updated : item)));
  ...
  return (
    <div>
      <PageHeader title="Thanh toán QR" subtitle="Tạo giao dịch VietQR để kiểm tra nhanh luồng thanh toán." />
      ...
      <div className="grid grid-2">
        <form className="card" onSubmit={handleCreate}>
          ...
          <div className="grid grid-2">
            <Input label="Ride ID" name="rideId" value={form.rideId} onChange={handleChange} placeholder="ride_1" required />
            <Input label="User ID" name="userId" value={form.userId} onChange={handleChange} placeholder="user_1 (phải trùng token)" />
            <Input
              label="Số tiền"
              name="amount"
              type="number"
              ...
              required
            />
            <Input label="Tiền tệ" name="currency" value={form.currency} onChange={handleChange} placeholder="VND" required />
            <Select label="Phương thức" name="method" value={form.method} onChange={handleChange}>
              <option value="VIETQR">VietQR (QR chuyển khoản)</option>
              <option value="PAYOS">PayOS (QR checkout)</option>
              <option value="CARD">Card</option>
            </Select>
            <Input label="Ghi chú" name="note" value={form.note} onChange={handleChange} placeholder="Thanh toán thử" />
          </div>
          <div style={{ marginTop: 12 }}>
            <label className="input-label">Idempotency-Key</label>
            <div className="qr-inline">
              <input className="input" name="idempotencyKey" value={idempotencyKey} onChange={(event) => setIdempotencyKey(event.target.value)} />
              <Button type="button" variant="ghost" onClick={() => setIdempotencyKey(buildIdemKey())}>
                Tạo key mới
              </Button>
            </div>
          </div>
          <div style={{ marginTop: 12 }}>
            ...
            <div className="qr-inline">
              ...
              <input
                ...
                onChange={(event) => setAutoConfirmDelay(event.target.value)}
                style={{ width: 90 }}
              />
              <span className="text-muted">giây</span>
            </div>
            <div className="input-helper">Dùng để test luồng trước khi tích hợp PayOS.</div>
          </div>
          <div style={{ marginTop: 12 }}>
            ...
            <div className="qr-inline">
              ...
              />
              <Button type="button" variant="ghost" onClick={() => setAuthToken(sessionToken || '')} disabled={!sessionToken}>
                Dùng token đăng nhập
              </Button>
            </div>
            {!authToken.trim() && <div className="input-helper">Chưa có token. Hãy đăng nhập admin hoặc dán JWT hợp lệ.</div>}
            {authToken.trim() && !tokenSubject && <div className="input-helper">Token không hợp lệ hoặc thiếu subject (sub).</div>}
            {tokenSubject && <div className="input-helper">Token subject: {tokenSubject}</div>}
            {import.meta.env.VITE_MOCK === 'true' && (
              <div className="input-helper">Đang bật mock. Token mock không dùng được cho payment-service thật.</div>
            )}
          </div>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <div className="qr-inline">
              <Button type="button" variant="outline" onClick={handleFetchQr} disabled={!payment?.id || !isVietQrPayment || loadingQr}>
                {loadingQr ? 'Đang tải...' : isVietQrPayment ? 'Lấy VietQR' : 'QR PayOS tự sinh'}
              </Button>
              <Button type="button" variant="ghost" onClick={handleConfirmDev} disabled={!payment?.id}>
                Xác nhận (dev)
              </Button>
            </div>
          </div>
          <div className="payment-sheet">
            <div className="payment-sheet-head">
              ...
              <div className="payment-sheet-brand-partners">
                <PayosLogo />
                <Badge variant={statusVariant(statusText)}>{labelFrom(paymentStatusLabels, statusText)}</Badge>
              </div>
            </div>
            ...
            <div className="payment-sheet-qr">
              <div className="payment-sheet-qr-frame">
                ...
                {qrImage ? (
                  <img src={qrImage} alt="Payment QR" className="payment-sheet-qr-image" />
                ) : (
                  <div className="payment-sheet-qr-empty">QR will appear right after payment is created.</div>
                )}
              </div>
            </div>
          ...
          {isPayosPayment && payment?.payos?.checkoutUrl && (
            <div style={{ marginTop: 12 }}>
              <Button type="button" variant="primary" onClick={() => window.open(payment.payos.checkoutUrl, '_blank', 'noopener,noreferrer')}>
                Open PayOS Checkout
              </Button>
            </div>
          )}
        ...
        <Table
          columns={[
            ...
            { key: 'userId', header: 'User' },
            {
              key: 'amount',
              header: 'Số tiền',
              render: (row) => (row.amount ? `${row.amount} ${row.currency}` : '-')
            },
            {
              key: 'method',
              header: 'Phương thức',
            ...
            },
            {
              key: 'status',
              header: 'Trạng thái',
              render: (row) => <Badge variant={statusVariant(row.status)}>{labelFrom(paymentStatusLabels, row.status)}</Badge>
            },
            {
              key: 'actions',
              header: 'Cập nhật',
              render: (row) => {
                ...
                return (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <select className="select" value={nextStatus} onChange={(event) => handleDraftChange(row.id, 'status', event.target.value)}>
                    ...
                    {nextStatus === 'FAILED' && (
                      <input
                        ...
                        onChange={(event) => handleDraftChange(row.id, 'failureReason', event.target.value)}
                        style={{ width: 160 }}
                      />
                    )}
                    <Button variant="ghost" type="button" onClick={() => handleUpdateStatus(row)}>
                      Lưu
                    </Button>
                  </div>
                );
