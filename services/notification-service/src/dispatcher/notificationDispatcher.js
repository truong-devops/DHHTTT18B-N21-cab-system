...
const DISPATCH_INTERVAL_MS = Number(process.env.NOTIFICATION_DISPATCH_INTERVAL_MS || 1000);
const DISPATCH_BATCH_SIZE = Number(process.env.NOTIFICATION_DISPATCH_BATCH_SIZE || 10);
const MAX_ATTEMPTS = Number(process.env.NOTIFICATION_MAX_ATTEMPTS || 5);
const BASE_BACKOFF_MS = Number(process.env.NOTIFICATION_RETRY_BASE_MS || 1000);
...
function canAttemptChannel(channelStatus, now) {
  ...
  if (!['PENDING', 'FAILED'].includes(channelStatus.status)) {
    return false;
  }
  if (channelStatus.attempts >= MAX_ATTEMPTS) {
    return false;
  }
  if (channelStatus.nextAttemptAt && channelStatus.nextAttemptAt > now) {
    return false;
  }
  return true;
}
...
async function updateOverallStatus(doc) {
  ...
  const overall = deriveOverallStatus(doc.perChannelStatus, doc.scheduledAt ? new Date(doc.scheduledAt) : null, new Date());
...
async function handleChannel(notification, channel) {
  const now = new Date();
  const channelStatus = notification.perChannelStatus?.[channel];

  if (!canAttemptChannel(channelStatus, now)) {
    return;
  }

  const claimed = await notificationRepository.claimChannel(notification.id, channel, MAX_ATTEMPTS, now);
  ...
  if (missingRecipient(channel, recipient)) {
    ...
    const updated = await notificationRepository.updateChannelResult(claimed.id, channel, {
      status: 'FAILED',
      processing: false,
  ...
  try {
    ...
    const attempts = claimed.perChannelStatus?.[channel]?.attempts || 1;
    const shouldRetry = attempts < MAX_ATTEMPTS;
    const nextAttemptAt = shouldRetry ? new Date(Date.now() + computeBackoff(attempts)) : null;
    const updated = await notificationRepository.updateChannelResult(claimed.id, channel, {
      status: 'FAILED',
      processing: false,
...
async function dispatchOnce() {
  ...
  const candidates = await notificationRepository.findDispatchCandidates(DISPATCH_BATCH_SIZE, now);
...
function startDispatcher() {
  ...
  const tick = async () => {
    ...
    try {
      ...
      logger.error({ err: error }, '[notification-service] dispatcher error');
    }
  };
