...
describe('auth middleware', () => {
  ...
  it('attaches user and userId from jwt', async () => {
    const token = jwt.sign({ sub: 'user-1', roles: ['driver'], scopes: ['read'] }, process.env.JWT_SECRET, { algorithm: 'HS256' });
  ...
  it('returns 403 when role missing', async () => {
    const token = jwt.sign({ sub: 'user-1', roles: ['rider'] }, process.env.JWT_SECRET, { algorithm: 'HS256' });
  ...
  it('allows when role present', async () => {
    const token = jwt.sign({ sub: 'user-1', roles: ['admin'] }, process.env.JWT_SECRET, { algorithm: 'HS256' });
    ...
    const app = buildApp((req, res, next) =>
      requireAuth(req, res, (err) => {
        ...
        return requireRole('admin')(req, res, () => res.json({ ok: true }));
      })
    );
  ...
  it('rejects when user does not match param', async () => {
    ...
    const response = await request(app).get('/users/user-2').set('Authorization', `Bearer ${token}`);
