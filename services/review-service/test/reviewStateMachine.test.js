const {
  isValidTransition,
  normalizeStatus
} = require("../src/domain/reviewStateMachine");
...
describe("review state machine", () => {
  ...
  it("accepts valid transitions", () => {
    expect(
      isValidTransition("SUBMITTED", "PUBLISHED")
    ).toBe(true);
    expect(
      isValidTransition("SUBMITTED", "REJECTED")
    ).toBe(true);
  ...
  it("rejects invalid transitions", () => {
    expect(
      isValidTransition("SUBMITTED", "COMPLETED")
    ).toBe(false);
    expect(
      isValidTransition("PUBLISHED", "REJECTED")
    ).toBe(false);
