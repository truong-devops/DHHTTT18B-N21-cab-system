...
describe('payment-service OpenAPI contract', () => {
  ...
  test('GET /v1/payments/{id} matches schema', async () => {
    ...
    const response = await request(app).get('/v1/payments/pay_1').set('Authorization', authHeader);
  ...
  test('POST /v1/payments 400 error matches schema', async () => {
    const response = await request(app).post('/v1/payments').set('Authorization', authHeader).set('Idempotency-Key', 'idem_2').send({ amount: 10 });
