...
function ensureVietQrConfig() {
  ...
  if (missing.length) {
    throw new ApiError(400, 'VIETQR_CONFIG_MISSING', `Missing VietQR config: ${missing.join(', ')}`);
  }
}
...
async function generateVietQr({ amount, currency, note, traceId, requestId, authorization }) {
  ...
  if (accountName.length < 5) {
    throw new ApiError(400, 'VIETQR_CONFIG_INVALID', 'VIETQR_ACCOUNT_NAME must be at least 5 characters after normalization');
  }
