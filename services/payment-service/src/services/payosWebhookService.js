...
async function handlePayosWebhook(payload) {
  ...
  if (amount !== null && paymentAmount !== null && amount !== paymentAmount) {
    log.warn({ orderCode, amount, paymentAmount }, 'PayOS webhook amount mismatch');
  }
  ...
  if ([STATUSES.PAID, STATUSES.FAILED, STATUSES.REFUNDED].includes(payment.status)) {
    ...
    log.warn({ orderCode, currentStatus: payment.status, targetStatus }, 'PayOS webhook ignored due to terminal status');
  ...
  log.info({ orderCode, status: updated.status, paymentId: updated.id }, 'PayOS webhook processed');
