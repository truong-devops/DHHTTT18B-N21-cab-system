...
const { claimPendingEvents, countOutboxBacklog, markPublished, markRetry, markDead } = require('../repository/outboxEventsRepository');
const logger = require('../utils/logger');
const monitoring = require('../monitoring');

const DEFAULT_INTERVAL_MS = Number(process.env.OUTBOX_PUBLISH_INTERVAL_MS || 5000);
const DEFAULT_BATCH_SIZE = Number(process.env.OUTBOX_PUBLISH_BATCH_SIZE || 50);
const WORKER_ID = process.env.OUTBOX_WORKER_ID || `${process.env.HOSTNAME || 'ride-service'}-${process.pid}`;
...
function resolvePartitionKey(row, payload) {
  return payload?.rideId || payload?.bookingId || row.aggregate_id || row.event_id;
}
...
async function moveToDlq(row, reason, traceId, payload) {
  ...
  await markDead(row.id, reason, dlqTopic, {
...
async function tick() {
  ...
  for (const row of rows) {
    ...
    if (!topic) {
      ...
      logger.error({ eventType: row.event_type, eventId: row.event_id }, '[ride-service] outbox unknown event_type');
    ...
    try {
      ...
      if (!result.published) {
        const retry = await markRetry(row.id, result.reason || 'validation_failed');
        ...
        if (retry?.status === 'dead') {
          await moveToDlq(row, 'validation_failed', traceId, payload);
        }
        monitoring.recordKafkaProcessingLatency({
          pipeline: 'outbox_publish',
          topic,
      ...
      logger.withTrace(traceId).error({ err: error, eventId: row.event_id, topic }, '[ride-service] outbox publish failed');

      const retry = await markRetry(row.id, error?.message || 'publish_failed');
      monitoring.recordKafkaRetry({
        scope: 'outbox',
        topic,
        status: String(retry?.status || 'unknown').toLowerCase(),
      ...
      if (retry?.status === 'dead') {
        await moveToDlq(row, error?.message || 'publish_failed', traceId, payload);
      }
      monitoring.recordKafkaProcessingLatency({
        pipeline: 'outbox_publish',
        topic,
...
function startOutboxPoller(intervalMs = DEFAULT_INTERVAL_MS) {
  const timer = setInterval(() => {
    tick().catch((error) => {
      logger.error({ err: error }, '[ride-service] outbox tick error');
    });
  }, intervalMs);
