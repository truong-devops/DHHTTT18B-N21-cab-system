...
jest.mock('../src/idempotency/store', () => {
  const store = new Map();
  return {
    buildIdempotencyKey: jest.fn(({ routeKey, userId, idempotencyKey }) => `idempo:${routeKey}:${userId}:${idempotencyKey}`),
    buildLockKey: jest.fn(({ routeKey, userId, idempotencyKey }) => `idempo:lock:${routeKey}:${userId}:${idempotencyKey}`),
    getCachedResponse: jest.fn((key) => Promise.resolve(store.get(key) || null)),
